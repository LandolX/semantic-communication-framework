...
class AWGNChannel(Channel):
    ...
    def propagate(self, signal):
        ...
        # Always recalculate noise standard deviation for each signal
        # This ensures correct noise power for signals with different power levels
...
class RayleighChannel(Channel):
    ...
    def propagate(self, signal):
        ...
        # Always recalculate for each signal to ensure correctness
...
class RicianChannel(Channel):
    ...
    def propagate(self, signal):
        ...
        if self.k_factor < 0:
            raise ValueError("K-factor cannot be negative")
...
class FrequencySelectiveChannel(Channel):
    ...
    def propagate(self, signal):
        ...
        if len(self.taps) == 0:
            raise ValueError("Channel taps cannot be empty")
        
        if len(self.delays) == 0:
            raise ValueError("Channel delays cannot be empty")
        
        if len(self.taps) != len(self.delays):
            raise ValueError("Number of taps must match number of delays")

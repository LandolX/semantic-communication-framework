...
class TestJPEG2000BGRDecoder:
    def setup_method(self):
        self.decoder = JPEG2000BGRDecoder()
    
    def test_decode_image(self):
        test_data = b'test_jpeg2000bgr_data'
        decoded_result = self.decoder.decode_image(test_data)
        assert decoded_result is not None
        # JPEG2000BGR解码器返回的是元组 (PIL Image, float)
    ...
    def test_decode_with_different_data(self):
        # 测试不同长度的数据
        test_data1 = b'short'
        test_data2 = b'longer test data for jpeg2000bgr decoder'

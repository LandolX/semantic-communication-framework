...
class JPEGDecoder:
    ...
    def _detect_frame_type(self, data):
        ...
        """
        if data.startswith(b'BLKJ'):
            return 'block_jpeg', struct.unpack('>I', data[4:8])[0], data[8:]
        elif data.startswith(b'JPEG'):
            return 'legacy_jpeg', struct.unpack('>I', data[4:8])[0], data[8:]
    ...
    def _decode_block_data(self, data, expected_size):
        ...
        try:
            # 使用通用分块解码器进行解码
            # 重新组装完整的编码数据（包括帧标记）
            full_data = b'BLKJ' + struct.pack('>I', expected_size) + data
            jpeg_data = self.block_codec.decode(full_data, 'jpeg')
            
            if jpeg_data and len(jpeg_data) <= expected_size:
                return jpeg_data
            elif jpeg_data:
                return jpeg_data[:expected_size]
            else:
                # 回退到简单的重复解码
    ...
    def _decode_legacy_data(self, data, expected_size):
        ...
        try:
            ...
            if len(data) >= encoded_header_size + encoded_body_size:
                ...
                if encoded_body_size > 0 and len(remaining) >= encoded_body_size:
                    encoded_body = remaining[:encoded_body_size]
                    header_decoded = simple_repeat_decode(encoded_header, 2)
    ...
    def _try_open_image(self, data):
        """
        尝试打开图像数据（自动检测格式）
        :param data: 图像格式的bytes数据（JPEG或PNG）
    ...
    def decode_image(self, framed_data, return_type='pil', default_size=(776, 776)):
        ...
        if frame_type == 'block_jpeg' and self.use_block_codec:
            print(f"检测到分块JPEG编码，原始大小: {original_size} bytes")
            # 使用完整的framed_data进行解码，而不是分离后的frame_data
            jpeg_data = self.block_codec.decode(framed_data, 'jpeg')
            
            if jpeg_data:
                print(f"通用分块解码成功，解码后数据大小: {len(jpeg_data)} bytes")
                image = self._try_open_image(jpeg_data)
                if image is not None:
                    ...
                    print(f"图像分块解码成功，恢复图像尺寸: {image.size}")
                    ...
                    if jpeg_data:
                        ...
                        if image is not None:
                            ...
                            print(f"图像分块解码成功（回退），恢复图像尺寸: {image.size}")
                            
                            if return_type == 'numpy':
                                return np.array(image), recovery_ratio
                            else:
                                return image, recovery_ratio
        elif frame_type == 'legacy_jpeg':
            print(f"检测到传统JPEG编码，原始大小: {original_size} bytes")
            ...
            if jpeg_data:
                ...
                if image is not None:
                    ...
                    print(f"图像传统解码成功，恢复图像尺寸: {image.size}")
    ...
    def decode_to_file(self, jpeg_data, output_path):
        ...
        """
        try:
            image, _ = self.decode_image(jpeg_data, return_type='pil')
    ...
    def verify_image_data(self, image_data):
        """
        验证图像数据是否有效（支持JPEG和PNG）
        :param image_data: 图像格式的bytes数据（JPEG或PNG）
...
def jpeg_verify(jpeg_data):
    """
    便捷的图像数据验证函数（支持JPEG和PNG）
    :param jpeg_data: 图像格式的bytes数据（JPEG或PNG）
    :return: 布尔值，True表示有效，False表示无效
    """
    decoder = JPEGDecoder()
    return decoder.verify_image_data(jpeg_data)
...
if __name__ == "__main__":
    ...
    from image_process.compression_coding.baseline.jpeg.jpeg_encoder import jpeg_encode_from_file
    
    test_image_path = os.path.join(project_root, 'data_input', 'image', '1-img-00000-00000_00002.png')
    
    if os.path.exists(test_image_path):
        # 使用从外部导入的jpeg_encode_from_file函数进行编码
        jpeg_data = jpeg_encode_from_file(test_image_path, quality=90, use_block_codec=False)
        print(f"传统编码: JPEG数据大小: {len(jpeg_data)} bytes")
        
        jpeg_data_block = jpeg_encode_from_file(test_image_path, quality=90, use_block_codec=True)
        print(f"分块编码: JPEG数据大小: {len(jpeg_data_block)} bytes")
        
        decoder = JPEGDecoder()
        image, ratio = decoder.decode_image(jpeg_data)
        print(f"传统解码恢复比例: {ratio:.2%}")
        
        decoder_block = JPEGDecoder(use_block_codec=True)
        image_block, ratio_block = decoder_block.decode_image(jpeg_data_block)

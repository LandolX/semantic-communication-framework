#!/usr/bin/env python3
"""
JPEG2000解码器 - 传统编码baseline实现
将JPEG2000格式的bytes数据解码为图像，与数字通信系统对齐
支持分块解码以提升无线传输的容错能力
"""
...
class JPEG2000Decoder:
    ...
    def _detect_frame_type(self, data):
        ...
        """
        if data.startswith(b'BLK2'):
            return 'block_jpeg2000', struct.unpack('>I', data[4:8])[0], data[8:]
        elif data.startswith(b'JP2K'):
            return 'legacy_jpeg2000', struct.unpack('>I', data[4:8])[0], data[8:]
    ...
    def _decode_block_data(self, data, expected_size):
        ...
        :return: 解码后的JPEG2000数据
        """
        try:
            # 使用通用分块解码器进行解码
            # 重新组装完整的编码数据（包括帧标记）
            full_data = b'BLK2' + struct.pack('>I', expected_size) + data
            jp2k_data = self.block_codec.decode(full_data, 'jpeg2000')
            
            if jp2k_data and len(jp2k_data) <= expected_size:
                return jp2k_data
            elif jp2k_data:
                return jp2k_data[:expected_size]
            else:
                # 回退到简单的重复解码
    ...
    def decode_image(self, framed_data, return_type='pil', default_size=(776, 776)):
        """
        解码包含FEC和交织的JPEG2000数据为图像，带有容错机制
        :param framed_data: 包含FEC和交织的JPEG2000数据
        ...
        if frame_type == 'block_jpeg2000' and self.use_block_codec:
            print(f"检测到分块JPEG2000编码，原始大小: {original_size} bytes")
            # 使用完整的framed_data进行解码，而不是分离后的frame_data
            jp2k_data = self.block_codec.decode(framed_data, 'jpeg2000')
            
            if jp2k_data:
                print(f"通用分块解码成功，解码后数据大小: {len(jp2k_data)} bytes")
                image = self._try_open_jpeg2000(jp2k_data)
                if image is not None:
                    ...
                    print(f"JPEG2000分块解码成功，恢复图像尺寸: {image.size}")
                    ...
                    print(f"JPEG2000分块解码失败：无法识别图像格式")
                    # 回退到简单的重复解码
                    jp2k_data = self._decode_block_data(frame_data, original_size)
                    if jp2k_data:
                        image = self._try_open_jpeg2000(jp2k_data)
                        if image is not None:
                            if image.mode != 'RGB':
                            ...
                            print(f"JPEG2000分块解码成功（回退），恢复图像尺寸: {image.size}")
                            
                            if return_type == 'numpy':
                                return np.array(image), recovery_ratio
                            else:
                                return image, recovery_ratio
        elif frame_type == 'legacy_jpeg2000':
            print(f"检测到传统JPEG2000编码，原始大小: {original_size} bytes")
            jp2k_data = self._decode_legacy_data(frame_data, original_size)
            ...
            if jp2k_data:
                ...
                if image is not None:
                    ...
                    print(f"JPEG2000传统解码成功，恢复图像尺寸: {image.size}")
                    ...
                    print(f"JPEG2000传统解码失败：无法识别图像格式")
    ...
    def decode_to_file(self, jp2k_data, output_path):
        """
        将JPEG2000格式的bytes数据解码并保存到文件
        :param jp2k_data: JPEG2000格式的bytes数据
        :param output_path: 输出文件路径
        :return: None
        """
        try:
            image, _ = self.decode_image(jp2k_data, return_type='pil')
    ...
    def verify_jpeg2000_data(self, jp2k_data):
        """
        验证JPEG2000数据是否有效
        :param jp2k_data: JPEG2000格式的bytes数据
        ...
        """
        try:
            buffer = io.BytesIO(jp2k_data)
            image = Image.open(buffer)
            image.load()
            buffer.close()
            return image.format in ['JPEG', 'JPEG2000']
        except Exception as e:
            print(f"JPEG2000数据验证失败: {e}")
            return False


def jpeg2000_decode(jp2k_data, return_type='pil'):
    """
    便捷的JPEG2000解码函数
    :param jp2k_data: JPEG2000格式的bytes数据
    :param return_type: 返回类型，'pil'返回PIL Image对象，'numpy'返回numpy数组
    ...
    return decoder.decode_image(jp2k_data, return_type)


def jpeg2000_decode_to_file(jp2k_data, output_path):
    """
    便捷的JPEG2000解码并保存到文件函数
    :param jp2k_data: JPEG2000格式的bytes数据
    :param output_path: 输出文件路径
    :return: None
    """
    decoder = JPEG2000Decoder()
    decoder.decode_to_file(jp2k_data, output_path)


def jpeg2000_verify(jp2k_data):
    """
    便捷的JPEG2000数据验证函数
    :param jp2k_data: JPEG2000格式的bytes数据
    :return: 布尔值，True表示有效，False表示无效
    """
    decoder = JPEG2000Decoder()
    return decoder.verify_jpeg2000_data(jp2k_data)
...
if __name__ == "__main__":
    ...
    from image_process.baseline.jpeg2000.jpeg2000_encoder import jpeg2000_encode_from_file
    
    test_image_path = os.path.join(project_root, 'data_input', 'image', '1-img-00000-00000_00002.png')
    
    if os.path.exists(test_image_path):
        # 使用从外部导入的jpeg2000_encode_from_file函数进行编码
        jp2k_data = jpeg2000_encode_from_file(test_image_path, quality=90, use_block_codec=False)
        print(f"传统编码: JPEG2000数据大小: {len(jp2k_data)} bytes")
        
        jp2k_data_block = jpeg2000_encode_from_file(test_image_path, quality=90, use_block_codec=True)
        print(f"分块编码: JPEG2000数据大小: {len(jp2k_data_block)} bytes")
        
        decoder = JPEG2000Decoder()
        image, ratio = decoder.decode_image(jp2k_data)
        print(f"传统解码恢复比例: {ratio:.2%}")
        
        decoder_block = JPEG2000Decoder(use_block_codec=True)
        image_block, ratio_block = decoder_block.decode_image(jp2k_data_block)

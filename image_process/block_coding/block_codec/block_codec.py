...
class BlockCodec:
    ...
    def decode(self, encoded_data, codec_type):
        ...
        try:
            # 解析帧标记
            original_size, block_size, fec_strategy, fec_level, frame_end = self._parse_frame_marker(encoded_data, codec_type)
            
            if original_size is None:
                return None
            
            # 创建临时BlockCodec实例，使用从帧标记中解析出的参数
            temp_codec = BlockCodec(
                block_size=block_size,
                fec_strategy=fec_strategy,
                fec_level=fec_level
            )
            
            # 解析编码块
            encoded_blocks = temp_codec._parse_encoded_blocks(encoded_data[frame_end:])
            
            if not encoded_blocks:
                return None
            
            # FEC解码
            blocks = temp_codec._recover_from_fec(encoded_blocks)
            
            if not blocks:
                return None
            
            # 重组数据
            data = temp_codec._assemble_from_blocks(blocks, original_size)
            ...
            try:
                print("尝试使用默认参数进行解码")
                default_codec = BlockCodec()
                # 简化的解析，只获取原始大小
                if len(encoded_data) >= 18:
                    original_size = struct.unpack('>I', encoded_data[4:8])[0]
                    # 尝试直接解析编码块
                    encoded_blocks = default_codec._parse_encoded_blocks(encoded_data[18:])
                    if encoded_blocks:
                        blocks = default_codec._recover_from_fec(encoded_blocks)
                        if blocks:
                            data = default_codec._assemble_from_blocks(blocks, original_size)
                            return data
            except Exception as e2:
                print(f"默认参数解码也失败: {e2}")
            return None
    ...
    def _parse_frame_marker(self, encoded_data, codec_type):
        ...
        :param codec_type: 编解码器类型
        :return: (原始大小, 分块大小, FEC策略, FEC级别, 有效载荷起始位置)，如果解析失败返回(None, None, None, None, None)
        """
        ...
        # 解析帧标记字段
        original_size = struct.unpack('>I', encoded_data[4:8])[0]
        block_size = struct.unpack('>I', encoded_data[8:12])[0]
        # 跳过保留字段
        fec_strategy_code = struct.unpack('B', encoded_data[16:17])[0]
        fec_level = struct.unpack('B', encoded_data[17:18])[0]
        
        # 转换FEC策略代码为字符串
        fec_strategy = 'repetition' if fec_strategy_code == 0 else 'xor'
        
        # 帧标记结束位置是18字节，后面跟着block_count和block_size
        return original_size, block_size, fec_strategy, fec_level, 18

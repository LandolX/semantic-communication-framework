...
将图像转换为JPEG格式的bytes数据，包含宏块分块和切片功能，每帧10个切片
"""
...
class JPEGEncoder:
    ...
    def encode_image(self, image, use_block_codec=None, lossless=False):
        ...
        try:
            ...
            # 保存为指定格式
            encoded_data = ImageUtils.save_image_to_buffer(image, format, self.quality)
            
            if use_block_codec:
                encoded_result = self._encode_with_block_codec(encoded_data)
                if self.quality >= 90:
                    print(f"编码后数据前10字节: {encoded_result[:10]}")
                    print(f"编码后数据大小: {len(encoded_result)} bytes")
                return encoded_result
            else:
                encoded_result = self._encode_legacy(encoded_data)
                if self.quality >= 90:
                    print(f"编码后数据前10字节: {encoded_result[:10]}")
                    print(f"编码后数据大小: {len(encoded_result)} bytes")
                return encoded_result
    ...
    def _encode_legacy(self, jpeg_data):
        ...
        if len(jpeg_data) > 100:
            ...
            fec_data = simple_repeat_encode(jpeg_data, 2)
